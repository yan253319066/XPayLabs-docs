...
description: "The XPayLabs API is a REST API with JSON request/response bodies secured by HMAC-SHA256 signing. Learn about the base URL, request format, response shape, and rate limits."
---
...
All requests must be signed with your merchant token using HMAC-SHA256. There are no API keys, Bearer tokens, or cookies involved. The merchant token is configured in your gateway and never transmitted over the network. See the [Authentication](/api-reference/authentication) page for the full signing reference.
