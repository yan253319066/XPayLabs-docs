...
keywords: ["Spring Boot 加密支付", "Java 加密支付 SDK", "加密支付 Java 集成", "xpay-java-sdk", "Maven 加密支付依赖", "Java Webhook 处理", "Spring Boot USDT 支付"]
---
...
| `baseUrl` | 否 | 网关基础 URL（默认：`https://api.xpaylabs.com`；自托管部署时覆盖此值）|
  ...
  public ResponseEntity<String> handleWebhook(@RequestBody String body) {
    ...
    switch (event.getNotifyType()) {
      ...
      case ORDER_PENDING_CONFIRMATION:
        System.out.println("已检测到支付，等待确认");
        break;
      ...
      case COLLECT_FAILED:
        ...
        System.out.println("归集失败：" + failedCollect.getReason());
        break;
    }
