...
keywords: ["XPayLabs", "Docker Compose 部署", "加密支付网关安装", "自托管支付网关搭建", "USDT 支付网关部署", "加密支付快速开始", "区块链支付部署教程"]
---
...
<Steps>
  <Step title="使用 Docker Compose 部署 XPayLabs">
    ...
    这将启动所有服务，包括 Java 后端（xpay-user、xpay-merchant、xpay-eth、xpay-tron、xpay-sui）、区块链扫描器和结账 UI。验证所有服务是否正常运行：
  ...
  <Step title="配置您的商家令牌">
    XPayLabs 使用 HMAC-SHA256 签名对 API 请求进行身份验证。您的商家令牌在创建商户账号时由网关管理面板生成。
