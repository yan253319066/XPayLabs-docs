...
keywords: ["crypto payment gateway", "self-hosted payment gateway", "non-custodial crypto payment", "multi-tenant payment infrastructure", "stablecoin payment processor", "USDT TRC20 gateway", "open source crypto payment", "TRON EVM SUI payment"]
---
...
**New: [x402 Protocol](/guides/x402-overview)** — XPayLabs is the first self-hosted gateway to support the x402 protocol for AI agent micropayments. Enable pay-per-call USDC billing for LLM APIs, AI agents, and any HTTP-based service — no API key management required.
